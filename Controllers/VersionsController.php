...
class Versions{
	...
	public function loadLanguages(){
		foreach( array_keys( $this->AllVersions ) as $abv ){
			...
			try{
				$Language = $this->SourceTextLibrary->getLanguage( $this->AllVersions[$abv]->getLanguageAbbreviation() );
	...
	public function loadVersions( string $VersionsFilePath ){
		try{
			$VersionList = $this->SourceTextLibrary->_loadJson( $VersionsFilePath );
		}catch( \Exception $e){
			echo $e->getMessage();
			exit(1);
		}
		...
		foreach( $VersionList as $abv => $v ){
			$tmpVersion = new Version( $this->SourceTextLibrary, $v );
